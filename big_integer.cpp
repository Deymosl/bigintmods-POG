...
BigInteger &BigInteger::operator>>=(int q) {
    fVector res = fVector(std::max(0, (int) data.size() - q / 32), 0);
    copy(q / 32, data.size(), 0, data, res);
    if (res.size() == 0)
        res.push_back(0);
    else {
        uint32_t next = 0;
...
std::string to_string(const BigInteger &q) {
    if (q == 0)
        return "0";
    std::string res;
    BigInteger w = q;
    while (w != 0) {
        BigInteger temp = w - (w / 10) * 10;
        res += '0' + temp.data[0];
        w.data = w.quotient(10);
        w.shrink();
    }
    if (q.sign)
        res.push_back('-');
